    ...
    def get_record(self):
        try:
            ...
            CommonUtil.raise_exception(
                handler_ins.request_url_path, ex.message, traceback.format_stack())
    ...
    def get_line_item_json(self, lineItemsArray):
        ...
        for lineItem in lineItemsArray:
            ...
            if lineItem.quantity is None:
                raise ZCRMException(self.request_url_path, APIConstants.RESPONSECODE_BAD_REQUEST,
                                    "Mandatory Field 'quantity' is missing.", None)
    ...
    def update_blueprint_data(self):
        try:
            ...
            try:
                ...
                from Utility import CommonUtil
            body = CommonUtil.create_api_supported_blueprint_json(
                input_json, APIConstants.BLUEPRINT, self.zcrmrecord.blueprint_transition_id)
                        ...
                        if isinstance(each_value.__getitem__(0), ZCRMInventoryLineItem):
                            ...
                            for every_pricebook_pricing in each_value:
                                pricebook_list.append(
                                    self.get_zcrmprice_detail_as_json(every_pricebook_pricing))
...
class RelatedListAPIHandler(APIHandler):
    ...
    def get_zcrm_note(self, note_details, zcrm_note_ins):
        if zcrm_note_ins is None:
            try:
                ...
                from Operations import ZCRMNote
            zcrm_note_ins = ZCRMNote.get_instance(
                self.parent_record, note_details['id'])
        ...
        zcrm_note_ins.created_by = ZCRMUser.get_instance(
            note_details['Created_By']['id'], note_details['Created_By']['name'])
        zcrm_note_ins.modified_by = ZCRMUser.get_instance(
            note_details['Modified_By']['id'], note_details['Modified_By']['name'])
    ...
    def get_zcrm_attachment(self, attachment_details):
        ...
        attachment_ins = ZCRMAttachment.get_instance(
            self.parent_record, attachment_details['id'])
        ...
        attachment_ins.owner = ZCRMUser.get_instance(
            attachment_details["Owner"]["id"], attachment_details["Owner"]["name"])
        attachment_ins.created_by = ZCRMUser.get_instance(
            attachment_details["Created_By"]["id"], attachment_details["Created_By"]["name"])
        attachment_ins.modified_by = ZCRMUser.get_instance(
            attachment_details["Modified_By"]["id"], attachment_details["Modified_By"]["name"])
    ...
    def upload_attachment(self, file_path):
        try:
            ...
            api_response_ins = APIRequest(
                handler_ins).upload_attachment(file_path)
...
class MassEntityAPIHandler(APIHandler):
    ...
    def get_records(self, cvid, sort_by, sort_order, page, per_page, custom_headers):
        try:
            ...
            handler_ins.add_param("per_page", per_page)
            bulk_api_response = APIRequest(handler_ins).get_bulk_api_response()
            data_arr = bulk_api_response.response_json[api_constant]
    ...
    def create_records(self, record_ins_list):
        try:
            ...
            if len(record_ins_list) > 100:
                CommonUtil.raise_exception(
                    'Records_Create', "records count must be less than or equals to 100", 'MORE RECORDS PROVIDED', "MORE RECORDS")
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.module_instance.api_name
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
            ...
            for record_ins in record_ins_list:
                if record_ins.entity_id is None:
                    data_array.append(EntityAPIHandler.get_instance(
                        record_ins).get_zcrmrecord_as_json())
                else:
                    CommonUtil.raise_exception(
                        'Records_Create', "record id must be None", 'RECORD ID PROVIDED', "RECORD ID")
            request_json = dict()
            ...
            for i in range(0, length):
                ...
                if entity_response_ins.status == APIConstants.STATUS_SUCCESS:
                    ...
                    EntityAPIHandler.get_instance(
                        new_record).set_record_properties(record_create_details)
    ...
    def upsert_records(self, record_ins_list, duplicate_check_fields):
        try:
            ...
            if len(record_ins_list) > 100:
                CommonUtil.raise_exception(
                    'Records_Upsert', "records count must be less than or equals to 100", 'MORE RECORDS PROVIDED', "MORE RECORDS")
            handler_ins = APIHandler()
            ...
            if (duplicate_check_fields is not None):
                ...
                handler_ins.add_param(
                    'duplicate_check_fields', duplicate_check_fields_as_string)
            data_array = list()
            for record_ins in record_ins_list:
    ...
    def update_records(self, record_ins_list):
        try:
            ...
            if len(record_ins_list) > 100:
                CommonUtil.raise_exception(
                    'Records_Update', "records count must be less than or equals to 100", 'MORE RECORDS PROVIDED', "MORE RECORDS")
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.module_instance.api_name
            handler_ins.request_method = APIConstants.REQUEST_METHOD_PUT
            handler_ins.request_api_key = APIConstants.DATA
    ...
    def update_mass_records(self, entityid_list, field_api_name, value):
        try:
            ...
            if len(entityid_list) > 100:
                CommonUtil.raise_exception(
                    'Records_Mass_Update', "entity id count must be less than or equals to 100", 'MORE RECORDS PROVIDED', "MORE RECORDS")
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.module_instance.api_name
            handler_ins.request_method = APIConstants.REQUEST_METHOD_PUT
            handler_ins.request_api_key = APIConstants.DATA
            ...
            for i in range(0, length):
                ...
                if entity_response_ins.status == APIConstants.STATUS_SUCCESS:
                    ...
                    updated_record = ZCRMRecord.get_instance(
                        self.module_instance.api_name, record_update_details['id'])
                    EntityAPIHandler.get_instance(
                        updated_record).set_record_properties(record_update_details)
    ...
    def delete_records(self, entityid_list):
        try:
            ...
            if len(entityid_list) > 100:
                CommonUtil.raise_exception(
                    'Records_delete', "entity id count must be less than or equals to 100", 'MORE RECORDS PROVIDED', "MORE RECORDS")
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.module_instance.api_name
            handler_ins.request_method = APIConstants.REQUEST_METHOD_DELETE
            handler_ins.request_api_key = APIConstants.DATA
            ids_as_string = ','.join(str(entity_id)
    ...
    def set_trash_record_properties(self, trash_record_ins, trash_record_prop):
        ...
        if 'created_by' in trash_record_prop and trash_record_prop['created_by'] is not None:
            trash_record_ins.created_by = ZCRMUser.get_instance(
                trash_record_prop['created_by']['id'], trash_record_prop['created_by']['name'])
        if 'deleted_by' in trash_record_prop and trash_record_prop['deleted_by'] is not None:
            trash_record_ins.deleted_by = ZCRMUser.get_instance(
                trash_record_prop['deleted_by']['id'], trash_record_prop['deleted_by']['name'])
...
class ModuleAPIHandler(APIHandler):
        ...
        try:
            if field_id is None:
                try:
                    ...
                    from Utility import CommonUtil
                CommonUtil.raise_exception(
                    'Field_GET', "field id must be given", 'FIELD ID IS NOT PROVIDED', "FIELD ID")
            handler_ins = APIHandler()
        ...
        try:
            if layout_id is None:
                try:
                    ...
                    from Utility import CommonUtil
                CommonUtil.raise_exception(
                    'Layout_GET', "layout id must be given", 'LAYOUT ID IS NOT PROVIDED', "LAYOUT ID")
            handler_ins = APIHandler()
        ...
        try:
            if customview_id is None:
                try:
                    ...
                    from Utility import CommonUtil
                CommonUtil.raise_exception(
                    'Customview_GET', "custom view id must be given", 'CUSTOM VIEW ID IS NOT PROVIDED', "CUSTOM VIEW ID")
            handler_ins = APIHandler()
            handler_ins.request_url_path = "settings/custom_views/" + \
                str(customview_id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_GET
    ...
    def get_zcrm_customview(self, customview_details, categories):
        ...
        customview_instance = ZCRMCustomView.get_instance(
            customview_details['id'], self.module_instance.api_name)
                    ...
                    if 'fields' in convert_map:
                        ...
                        for each_field_data in field_data:
                            convert_mapping_field_ins = ZCRMLeadConvertMappingField.get_instance(
                                each_field_data['api_name'], each_field_data['id'])
    ...
    def get_zcrmfield(self, field_details):
        ...
        field_instance.sequence_number = int(
            field_details['sequence_number']) if 'sequence_number' in field_details else None
        ...
        field_instance.is_custom_field = bool(
            field_details['custom_field']) if 'custom_field' in field_details else None
        ...
        field_instance.is_read_only = bool(
            field_details['read_only']) if 'read_only' in field_details else None
        field_instance.is_business_card_supported = bool(
            field_details['businesscard_supported']) if 'businesscard_supported' in field_details else None
        ...
        if 'formula' in field_details and len(field_details['formula']) > 0:
            ...
            field_instance.formula_expression = field_details['formula'][
                'expression'] if 'expression' in field_details['formula'] else None
        ...
        if 'currency' in field_details and len(field_details['currency']) > 0:
            ...
            field_instance.precision = int(
                field_details['currency']['precision']) if 'precision' in field_details['currency'] else None
            field_instance.rounding_option = field_details['currency'][
                'rounding_option'] if 'rounding_option' in field_details['currency'] else None
        ...
        if 'auto_number' in field_details and len(field_details['auto_number']) > 0:
            ...
            field_instance.start_number = field_details['auto_number'][
                'start_number'] if 'start_number' in field_details['auto_number'] else None

        field_instance.display_label = field_details[
            'display_label'] if 'display_label' in field_details else None
        field_instance.field_label = field_details['field_label'] if 'field_label' in field_details else None
        ...
        field_instance.is_webhook = bool(
            field_details['webhook']) if 'webhook' in field_details else None
        ...
        field_instance.is_history_tracking = bool(
            field_details['history_tracking']) if 'history_tracking' in field_details else None
        field_instance.is_system_mandatory = bool(
            field_details['system_mandatory']) if 'system_mandatory' in field_details else None

        if 'related_details' in field_details and len(field_details['related_details']) > 0:
            field_instance.related_details = self.get_lookup_field_instance(
                field_details['related_details'])
        field_instance.transition_sequence = field_details[
            'transition_sequence'] if 'transition_sequence' in field_details else None
    ...
    def get_lookup_field_instance(self, lookup_field_details):
        ...
        lookup_field_instance.display_label = lookup_field_details[
            'display_label'] if 'display_label' in lookup_field_details else None
        ...
        if (isinstance(lookup_field_details['module'], dict)):
            ...
            relatedmodule_instance = ZCRMBlueprintRelatedModule.get_instance(
                moduledict['id'])
            relatedmodule_instance.display_label = moduledict[
                'display_label'] if 'display_label' in moduledict else None
            relatedmodule_instance.module_name = moduledict[
                'module_name'] if 'display_label' in moduledict else None
            lookup_field_instance.module = relatedmodule_instance
        else:
            lookup_field_instance.module = lookup_field_details['module']
        lookup_field_instance.type = lookup_field_details[
            '_type'] if '_type' in lookup_field_details else None
        lookup_field_instance.field_label = lookup_field_details[
            'field_label'] if 'field_label' in lookup_field_details else None
...
class MetaDataAPIHandler(APIHandler):
    ...
    def get_zcrmmodule(self, module_details):
        ...
        crmmodule_instance.business_card_field_limit = int(
            module_details['business_card_field_limit']) if 'business_card_field_limit' in module_details else None
        ...
        crmmodule_instance.is_global_search_supported = bool(
            module_details['global_search_supported']) if 'global_search_supported' in module_details else None
        try:
        ...
        if module_details['modified_by'] is not None:
            crmmodule_instance.modified_by = ZCRMUser.get_instance(
                module_details['modified_by']["id"], module_details['modified_by']["name"])
        ...
        if 'related_lists' in module_details and module_details['related_lists'] is not None:
            ...
            for relatedlist in relatedlists:
                ...
                relatedlist_instances.append(
                    module_relatedlist_ins.set_relatedlist_properties(relatedlist))
        ...
        if 'layouts' in module_details and module_details['layouts'] is not None:
            crmmodule_instance.layouts = ModuleAPIHandler.get_instance(ZCRMModule.get_instance(
                module_details[APIConstants.API_NAME])).get_zcrmlayouts(module_details['layouts'])

        if 'fields' in module_details and module_details['fields'] is not None:
            crmmodule_instance.fields = ModuleAPIHandler.get_instance(ZCRMModule.get_instance(
                module_details[APIConstants.API_NAME])).get_zcrmfields(module_details['fields'])
        ...
        if 'custom_view' in module_details and module_details['custom_view'] is not None:
            crmmodule_instance.default_custom_view = ModuleAPIHandler.get_instance(ZCRMModule.get_instance(
                module_details[APIConstants.API_NAME])).get_zcrm_customview(module_details['custom_view'], None)
...
class OrganizationAPIHandler(APIHandler):
    ...
    def get_role(self, role_id):
        try:
            ...
            if role_id is None:
                CommonUtil.raise_exception(
                    'Role_GET', "role id must be given", 'ROLE ID IS NOT PROVIDED', "ROLE ID")
            handler_ins = APIHandler()
    ...
    def get_profile(self, profile_id):
        try:
            ...
            if profile_id is None:
                CommonUtil.raise_exception(
                    'Profile_GET', "profile id must be given", 'PROFILE ID IS NOT PROVIDED', "PROFILE ID")
            handler_ins = APIHandler()
    ...
    def create_user(self, user_instance):
        try:
            ...
            if user_instance is None:
                CommonUtil.raise_exception(
                    'User_POST', "user instance must be given", 'USER INSTANCE IS NOT PROVIDED', "USER INSTANCE ID")
            handler_ins = APIHandler()
    ...
    def update_user(self, user_instance):
        try:
            ...
            if user_instance is None or user_instance.id is None:
                CommonUtil.raise_exception('User_PUT', "user instance and id must be given",
    ...
    def delete_user(self, user_id):
        try:
            ...
            if user_id is None:
                CommonUtil.raise_exception(
                    'User_DELETE', "user id must be given", 'USER ID IS NOT PROVIDED', "USER ID")
            handler_ins = APIHandler()
    ...
    def get_user(self, user_id):
        try:
            ...
            if user_id is None:
                CommonUtil.raise_exception(
                    'User_GET', "user id must be given", 'USER ID IS NOT PROVIDED', "USER ID")
            handler_ins = APIHandler()
            handler_ins.request_url_path = "users/"+str(user_id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_GET
            handler_ins.request_api_key = APIConstants.USERS
    ...
    def get_organization_taxes(self):
        try:
            ...
            for org_tax_details in org_tax_json:
                org_tax_instances.append(
                    self.get_zcrm_org_tax(org_tax_details))
    ...
    def get_organization_tax(self, org_tax_id):
        try:
            ...
            handler_ins.request_api_key = APIConstants.TAXES
            api_response = APIRequest(handler_ins).get_api_response()
            api_response.data = self.get_zcrm_org_tax(
                api_response.response_json[APIConstants.TAXES][0])
            return api_response
        except ZCRMException as ex:
            raise ex
        except Exception as ex:
        ...
        try:
            ...
            for orgtax_instance in orgtax_instances:
                if orgtax_instance.id is None:
                    org_tax_array.append(
                        self.get_zcrmorgtax_as_json(orgtax_instance))
                else:
    ...
    def delete_organization_taxes(self, orgtax_ids):
        try:
            ...
            handler_ins.request_method = APIConstants.REQUEST_METHOD_DELETE
            handler_ins.request_api_key = APIConstants.TAXES
            ids_as_string = ','.join(str(orgtax_id)
                                     for orgtax_id in orgtax_ids)
            handler_ins.add_param('ids', ids_as_string)
    ...
    def get_zcrmorgtax_as_json(self, orgtax_instance):
        ...
        if orgtax_instance.display_label is not None:
            orgtax_as_json['display_label'] = str(
                orgtax_instance.display_label)
        ...
        if orgtax_instance.sequence_number is not None:
            orgtax_as_json['sequence_number'] = str(
                orgtax_instance.sequence_number)
    ...
    def get_zcrm_org_tax(self, orgtax_details):
        ...
        org_tax_instance = ZCRMOrgTax.get_instance(
            orgtax_details['id'], orgtax_details['name'])
    ...
    def get_zcrm_role(self, role_details):
        try:
            ...
            from Operations import ZCRMRole, ZCRMUser
        role_instance = ZCRMRole.get_instance(
            role_details['id'], role_details['name'])
        ...
        if 'reporting_to' in role_details and role_details['reporting_to'] is not None:
            role_instance.reporting_to = ZCRMRole.get_instance(
                role_details['reporting_to']['id'], role_details['reporting_to']['name'])
        if 'forecast_manager' in role_details and role_details['forecast_manager'] is not None:
            role_instance.forecast_manager = ZCRMUser.get_instance(
                role_details['forecast_manager']['id'], role_details['forecast_manager']['name'])
    ...
    def get_zcrm_profile(self, profile_details):
        try:
            ...
            from Operations import ZCRMProfile, ZCRMUser, ZCRMPermission, ZCRMProfileSection, ZCRMProfileCategory
        profile_instance = ZCRMProfile.get_instance(
            profile_details['id'], profile_details['name'])
        ...
        if profile_details['modified_by'] is not None:
            profile_instance.modified_by = ZCRMUser.get_instance(
                profile_details['modified_by']['id'], profile_details['modified_by']['name'])
        if profile_details['created_by'] is not None:
            profile_instance.created_by = ZCRMUser.get_instance(
                profile_details['created_by']['id'], profile_details['created_by']['name'])
    ...
    def get_zcrm_user(self, user_details):
        ...
        user_instance = ZCRMUser.get_instance(
            user_details['id'], user_details['name'] if 'name' in user_details else None)
        ...
        user_instance.is_personal_account = bool(
            user_details['personal_account']) if 'personal_account' in user_details else None
    ...
    def get_zcrm_user_customizeinfo(self, customize_info):
        ...
        customize_info_instance.is_to_show_right_panel = bool(
            customize_info['show_right_panel']) if 'show_right_panel' in customize_info else None
        customize_info_instance.is_bc_view = customize_info['bc_view']
        customize_info_instance.is_to_show_home = bool(
            customize_info['show_home']) if 'show_home' in customize_info else None
        customize_info_instance.is_to_show_detail_view = bool(
            customize_info['show_detail_view']) if 'show_detail_view' in customize_info else None
...
class TagAPIHandler(APIHandler):
    ...
    def add_tags_to_multiple_records(self, tag_list, record_list):
        try:
            ...
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
            record_ids_as_string = ','.join(
                str(record_id) for record_id in record_list)
            tag_names_as_string = ','.join(
                str(tag_name) for tag_name in tag_list)
            ...
            for i in range(0, length):
                ...
                if entity_response_ins.status == APIConstants.STATUS_SUCCESS:
                    ...
                    record = ZCRMRecord.get_instance(
                        self.module_instance.api_name, tag_added_details['id'])
    ...
    def add_tags(self, record, tag_list):
        try:
            ...
            handler_ins = APIHandler()
            handler_ins.request_url_path = record.module_api_name + \
                '/'+record.entity_id + '/actions/add_tags'
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
            tag_names_as_string = ','.join(
                str(tag_name) for tag_name in tag_list)
            ...
            EntityAPIHandler.get_instance(
                record).set_record_properties(response_details)
    ...
    def remove_tags(self, record, tag_list):
        try:
            ...
            handler_ins = APIHandler()
            handler_ins.request_url_path = record.module_api_name + \
                '/'+record.entity_id + '/actions/remove_tags'
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
            tag_names_as_string = ','.join(
                str(tag_name) for tag_name in tag_list)
    ...
    def delete(self, tag_id):
        try:
            ...
            if tag_id is None:
                CommonUtil.raise_exception(
                    'delete', "no tag id given", 'NO TAG ID PROVIDED', "NO TAG ID")
            handler_ins = APIHandler()
    ...
    def merge(self, tag_id, merge_tag_id):
        try:
            ...
            if tag_id is None:
                CommonUtil.raise_exception(
                    'merge', "no tag id given", 'NO TAG ID PROVIDED', "NO TAG ID")
            if merge_tag_id is None:
                CommonUtil.raise_exception(
                    'merge', "no merge tag id given", 'NO MERGE TAG ID PROVIDED', "NO MERGE TAG ID")
            handler_ins = APIHandler()
    ...
    def update(self, tag):
        try:
            ...
            if tag is None:
                CommonUtil.raise_exception(
                    'update', "no tag provided", 'NO TAG  PROVIDED', "NO TAG ")
    ...
    def get_zcrmtag(self, tag_details):
        ...
        if 'created_by' in tag_details and tag_details['created_by'] is not None:
            tag_instance.created_by = ZCRMUser.get_instance(
                tag_details['created_by']['id'], tag_details['created_by']['name'])
        if 'modified_by' in tag_details and tag_details['modified_by'] is not None:
            tag_instance.modified_by = ZCRMUser.get_instance(
                tag_details['modified_by']['id'], tag_details['modified_by']['name'])
    ...
    def get_variable_groups(self):
        ...
        try:
            ...
            for json_data in data:
                ...
                self.get_variable_groups_res_as_obj(
                    json_data, variable_groups_ins)
...
class BlueprintAPIHandler(object):
    ...
    def update(self):
        try:
            apiresponse = EntityAPIHandler.get_instance(
                self.zcrmrecord).update_blueprint_data()
            apiresponse.data = self.zcrmrecord
            return apiresponse
        except ZCRMException as ex:
            raise ex
    ...
    def get(self):
        try:
            apiresponse = EntityAPIHandler.get_instance(
                self.zcrmrecord).get_blueprint_data()
            return apiresponse
        except ZCRMException as ex:
            raise ex
